...
public class Program
{
    ...
    private static void SendKey(byte vk, bool ctrl = false, bool shift = false)
    {
    ...
    public static async Task Main(string[] args)
    {
        ...
        {
            await RunAntigravityEmpiricalStudyAsync(args);
        }
    ...
    private record AntigravityTelemetryStep(
        int StepNumber,
        string ZoneTag,
        string Description,
        string Stimulus,
    ...
    private static async Task RunAntigravityEmpiricalStudyAsync(string[] args)
    {
        Console.ForegroundColor = ConsoleColor.Cyan;
        Console.WriteLine("==========================================================================");
        ...
        var target = candidates.FirstOrDefault(c => c.Title.Contains("active-desktop-context-engine", StringComparison.OrdinalIgnoreCase) && !IsIconic(c.Hwnd))
                     ?? candidates.FirstOrDefault(c => !IsIconic(c.Hwnd))
                     ?? candidates[0];
        ...
        SendKey(VK_E, ctrl: true, shift: true);
        ...
        if (breadcrumb == null)
        {
            SendKey(VK_OEM_1, ctrl: true, shift: true);
            await Task.Delay(500);
